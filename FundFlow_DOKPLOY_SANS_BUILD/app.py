...
def resolve_data_dir() -> Path:
    ...
    Sur Dokploy, DATA_DIR peut pointer vers un volume persistant. Si ce chemin
    n'est pas accessible, l'application utilise /tmp afin de démarrer au lieu
    de planter au lancement.
    """
    requested = Path(os.environ.get("DATA_DIR", str(APP_DIR / "data"))).expanduser()
    candidates = [requested, Path("/tmp/fundflow_data")]
    ...
    for candidate in candidates:
        try:
            ...
            last_error = exc
    raise RuntimeError(f"Aucun dossier de données accessible: {last_error}")
...
APP_NAME = "FundFlow"
DEFAULT_LOGO = ""
DEFAULT_ADMIN_EMAIL = os.environ.get("ADMIN_EMAIL", "admin@fundflow.local").strip().lower()
...
def seed_users() -> None:
    """Crée le premier administrateur sans publier un mot de passe dans le code.

    En production, ADMIN_EMAIL et ADMIN_PASSWORD doivent être définis dans
    Dokploy. Si ADMIN_PASSWORD est absent lors du tout premier démarrage, un
    mot de passe aléatoire est généré et affiché une seule fois dans les logs.
    Les comptes de démonstration ne sont créés que si SEED_DEMO_USERS=true.
    """
    admin = query_one("SELECT id FROM users WHERE lower(email)=?", (DEFAULT_ADMIN_EMAIL,))
    if not admin:
        configured_password = os.environ.get("ADMIN_PASSWORD", "").strip()
        generated_password = configured_password or secrets.token_urlsafe(14)
        execute(
        ...
        if not configured_password:
            print("=" * 60)
            print("PREMIER DEMARRAGE - IDENTIFIANTS ADMIN GENERES")
            print(f"Email administrateur : {DEFAULT_ADMIN_EMAIL}")
            print(f"Mot de passe temporaire : {generated_password}")
            print("Connectez-vous puis changez immédiatement ce mot de passe.")
            print("=" * 60)

    seed_demo = os.environ.get("SEED_DEMO_USERS", "false").strip().lower() in {"1", "true", "yes", "oui"}
    if not seed_demo:
        return
    ...
    for name, email, pwd, role, dep, job_title in demo_users:
